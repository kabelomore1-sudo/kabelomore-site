...
import { Loader2, Send, CircleCheck, AlertTriangle } from "lucide-react";
...
/**
 * Fallback intake form rendered when a tier doesn't yet have a Tally form ID.
 * Captures the basics (business + contact + tier) and routes to /api/scan.
 * Kabelo follows up within 1 hour with the full brief link.
 */
export function BriefFallback({
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      // Tag this as a brief request (not a free scan), with tier
      tier: tierId,
    };
    ...
    return (
      <div className="rounded-3xl border border-accent-200 bg-accent-50 p-8 text-center md:p-12">
        ...
        <h2 className="mt-6 text-2xl font-semibold tracking-tight text-ink-900">
          Got it — Kabelo&apos;s on it.
        </h2>
        <p className="mx-auto mt-3 max-w-md text-base text-ink-700 leading-relaxed">
          Within 1 hour you&apos;ll receive an email with the full structured brief link
          + the deposit invoice for {tierName}. If you don&apos;t see it, check spam, or
          email kabelo@kabelomore.com directly.
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-3xl border border-rule bg-white p-8 shadow-soft md:p-12">
      <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
        ...
        <div className="text-sm text-ink-700 leading-relaxed">
          <span className="font-semibold text-ink-900">Quick start while we finish setting up:</span>{" "}
          Submit your basics here. Within 1 hour you&apos;ll get an email with the full
          structured brief and the deposit invoice for{" "}
          <span className="font-semibold text-ink-900">{tierName}</span>. No payment until
          you&apos;ve seen and approved the brief.
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-5" noValidate>
        ...
        <div>
          ...
          <textarea
            ...
            name="services"
            rows={3}
            placeholder="e.g. timeline pressure, specific competitor I want to beat, services I'm prioritising"
        ...
        <div className="flex flex-col items-start gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-ink-500">
            No deposit until you approve the brief. We never share your details.
          </p>
          <Button
            type="submit"
            variant="primary"
          ...
          >
            {state.status === "submitting" ? (
              ...
              <>
                Get the full brief <Send className="h-4 w-4" />
              </>
            )}
          </Button>
        </div>
